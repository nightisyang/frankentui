...
use ftui::widgets::{List, ListItem, ListState, Table, TableState, Row};
use ftui::layout::Constraint;
use ftui::render::{Frame, buffer::Buffer, grapheme_pool::GraphemePool};
...
fn table_fills_viewport_on_resize() {
    ...
    let rows: Vec<Row> = (0..10).map(|i| Row::new(vec![format!("Row {}", i)])).collect();
...
fn table_fills_viewport_with_variable_heights() {
    ...
    let mut rows: Vec<Row> = (0..9).map(|i| Row::new(vec![format!("Row {}", i)])).collect();
