            ...
            if let Ok(input) = doc.create_element("textarea") {
                if let Ok(input) = input.dyn_into::<HtmlTextAreaElement>() {
                    ...
                    if let Some(body) = doc.body() {
                        ...
                        let _ = canvas.add_event_listener_with_callback("touchstart", closure.as_ref().unchecked_ref());
    ...
    impl Drop for FrankenTermWeb {
        fn drop(&mut self) {
            if let (Some(canvas), Some(closure)) = (&self.canvas, &self.on_canvas_touch) {
                let _ = canvas.remove_event_listener_with_callback("touchstart", closure.as_ref().unchecked_ref());
                let _ = canvas.remove_event_listener_with_callback("mousedown", closure.as_ref().unchecked_ref());
            }
